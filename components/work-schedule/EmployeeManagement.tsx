      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-200">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
            <h2 className="text-lg font-semibold text-gray-900">직원 목록 ({employees.filter(emp => !emp.resignationDate).length}명)</h2>
                          ...
                          <div className="space-y-1">
                            {[...new Set(employee.branchNames)].map((branchName, index) => (
                              <div key={index} className="text-gray-600">
                                {branchName === employee.primaryBranchName ? '⭐ ' : ''}{branchName}
                              </div>
                            ))}
                          </div>
                        ) : '-'}
